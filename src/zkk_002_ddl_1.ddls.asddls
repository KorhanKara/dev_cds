@AbapCatalog.sqlViewName: 'ZKK_002_CDS_1'
@AbapCatalog.compiler.compareFilter: true
@AbapCatalog.preserveKey: true
@AccessControl.authorizationCheck: #NOT_REQUIRED
@EndUserText.label: 'İkinci Hafta Ödev 1'
define view ZKK_002_DDL_1 as select from vbrp
...
currency_conversion(amount=>vbrp.netwr,source_currency=>vbrk.waerk,target_currency=>cast('EUR' as abap.cuky(5)),exchange_rate_date=>vbrk.fkdat) as conversion_netwr,
    ...
    length(makt.matnr) as matnr_length,
    case vbrk.fkart when 'FAS' then 'Peşinat talebi iptali' when 'FAZ' then 'Peşinat talebi' else 'Fatura' end as Faturalama_Turu,
    vbrk.fkdat,
    vbrk.inco2_l
